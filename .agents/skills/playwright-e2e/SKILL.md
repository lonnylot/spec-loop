...
description: >
  ...
  flow tests, Playwright webServer env, blanking live API keys, an
...
3. **Seams in the test server.** The documented test adapter constructs auth, persistence, and any model/HTTP fakes **once per Node process** so register, login, and later requests in the same worker still see that state (`codebase-design` composition root). Keep that process-global state **inside one test** so a multi-step flow still sees the account. Reset the fake store/mail **between files** so an empty-state spec still passes after a mutating file has run. Playwright's webServer env blanks live API keys so the suite cannot reach third-party hosts. Point any file database at a temp path. Completion: a second request in the same worker still sees the registered account; empty-state still passes after a mutating file ran (file order does not matter); `workers: 1` is not that proof; the spec does not call the live vendor. A leftover identifier in wiring source is not this proof (`testing`).
