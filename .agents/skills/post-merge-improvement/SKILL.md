...
- `release-manager` just merged a PR
...
4. **Push and open a PR** for the learning branch. Run `spec-review-loop` on that PR until Approve + green CI, then dispatch `release-manager`. Skills/docs PRs are not exempt. Do not merge in the orchestrator session.
